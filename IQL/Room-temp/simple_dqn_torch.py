 ...
 #       action4 = T.argmax(actions4).item()
    ...
    def choose_action(self, observation, action_set, player):
        if np.random.random() > self.epsilon:
            ...
            idx2 = np.random.choice(T.tensor(list(range(len(action_set[0][0])))).to(self.Q_eval.device))
  #          idx3 = np.random.choice(T.tensor(list(range(len(action_set[0][0])))).to(self.Q_eval.device))
  #          idx4 = np.random.choice(T.tensor(list(range(len(action_set[0][0])))).to(self.Q_eval.device))
            action =  [np.array([action_set[0][0][idx1], action_set[0][0][idx2]], dtype=np.int64)]
